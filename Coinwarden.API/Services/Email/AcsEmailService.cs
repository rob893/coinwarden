...
public sealed class AcsEmailService : IEmailService
{
    ...
    public async Task SendEmailConfirmationToUserAsync(User user, string token, CancellationToken cancellationToken)
    {
        ...
        await this.SendEmailToUserAsync(user, "Coinwarden Email Confirmation - Verify Your Account! 📧", plainTextMessage, htmlMessage, cancellationToken);
    }
    ...
    public async Task SendResetPasswordToUserAsync(User user, string token, CancellationToken cancellationToken)
    {
        ...
        await this.SendEmailToUserAsync(user, "Coinwarden Password Reset - Let's Get You Back In! 🔑", plainTextMessage, htmlMessage, cancellationToken);
    }
